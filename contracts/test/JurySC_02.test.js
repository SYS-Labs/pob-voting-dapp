...
describe("JurySC_02 (address-based projects)", function () {
  ...
  describe("voting", function () {
    ...
    it("prevents a registered project from being assigned as DevRel", async function () {
      // Role isolation: projects cannot be DevRel
      await expect(
        jurySC.connect(owner).setDevRelAccount(project1.address)
      ).to.be.revertedWithCustomError(jurySC, "DevRelCannotBeProject");
    });

    it("prevents a registered project from being added as DAO_HIC", async function () {
      // Role isolation: projects cannot be DAO_HIC voters
      await expect(
        jurySC.connect(owner).addDaoHicVoter(project1.address)
      ).to.be.revertedWithCustomError(jurySC, "DaoHicCannotBeProject");
    });
  ...
  describe("dual voting modes (CONSENSUS vs WEIGHTED)", function () {
    ...
    describe("scenario: no entity consensus, community split, devrel votes top community choice", function () {
      beforeEach(async function () {
        ...
        // Community votes (4 voters total):
        // - 2 votes for Project 1
        // - 1 vote for Project 2
        // - 1 vote for Project 3
        // Top choice is Project 1
        ...
        // Community voter 4 votes for Project 3
        const tx4 = await pob.connect(community4).mint({ value: DEPOSIT });
        const receipt4 = await tx4.wait();
        const tokenId4 = receipt4.logs[0].args[2];
        await jurySC.connect(community4).voteCommunity(tokenId4, project3.address);
      ...
      it("CONSENSUS mode: Project 1 wins (2-out-of-3 agreement)", async function () {
        // DevRel → Project 1
        // DAO_HIC → Project 3 (majority)
        // Community → Project 1 (2 votes vs 1 each for P2 and P3)
        // Project 1 has 2 entity votes (DevRel + Community)
      ...
      it("WEIGHTED mode: Project 1 wins with highest score", async function () {
        ...
        //   DevRel: 0
        //   DAO_HIC: 0
        //   Community: 1/4 * 1/3 = 1/12
        //   Total: 1/12 ≈ 83333333333333333 wei

        // Project 3 score:
        //   DevRel: 0
        //   DAO_HIC: 3/3 * 1/3 = 1/3 (333333333333333333 wei)
        //   Community: 1/4 * 1/3 = 1/12 (83333333333333333 wei)
        //   Total: 5/12 ≈ 416666666666666666 wei
        ...
        // P1: 1/3 + 2/4 * 1/3 = 1/3 + 1/6 = 1/2 * 1e18 = 499999999999999999
        expect(p1Score).to.be.closeTo(499999999999999999n, 10n);

        // P2: 1/4 * 1/3 = 1/12 * 1e18 ≈ 83333333333333333
        expect(p2Score).to.be.closeTo(83333333333333333n, 10n);

        // P3: 1/3 + 1/4 * 1/3 = 1/3 + 1/12 = 5/12 * 1e18 ≈ 416666666666666666
        expect(p3Score).to.be.closeTo(416666666666666666n, 10n);
      });
      ...
      it("getWinner() uses CONSENSUS mode by default", async function () {
        ...
        expect(hasWinner).to.be.true; // Project 1 has consensus
        expect(winner).to.equal(project1.address);
      });
    });
  ...
  describe("Security fixes", function () {
    ...
    describe("MEDIUM-01 Fix: NFT transfer restrictions", function () {
      it("Should block NFT transfers during active voting", async function () {
        // Community member mints NFT and votes
        await pob.connect(community1).mint({ value: DEPOSIT });
        await jurySC.connect(community1).voteCommunity(0, project1.address);

        // Try to transfer NFT during active voting - should fail
        await expect(
      ...
      it("Should allow NFT transfers after voting ends", async function () {
        // Community member mints NFT and votes
        await pob.connect(community1).mint({ value: DEPOSIT });
        await jurySC.connect(community1).voteCommunity(0, project1.address);
        ...
        await expect(pob.connect(community1).transferFrom(community1.address, community2.address, 0)).to.not.be
          .reverted;
      ...
      it("Should allow minting during voting (not a transfer)", async function () {
        // Minting should work (from = address(0))
        await expect(pob.connect(community1).mint({ value: DEPOSIT })).to.not.be.reverted;
        expect(await pob.ownerOf(0)).to.equal(community1.address);
      });

      it("Verifies one account cannot vote multiple times via transfers", async function () {
        // This test verifies the security fix works end-to-end
        // community1 mints and votes
        await pob.connect(community1).mint({ value: DEPOSIT });
        await jurySC.connect(community1).voteCommunity(0, project1.address);

        // Verify vote was recorded
        expect(await jurySC.communityVoteOf(0)).to.equal(project1.address);
        expect(await jurySC.communityVotesCast()).to.equal(1);

        // Transfer is blocked during voting, so community1 cannot acquire another NFT
        // and vote again (attack prevented)
        await expect(
          pob.connect(community1).transferFrom(community1.address, community2.address, 0)
        ).to.be.revertedWithCustomError(pob, "TransferDuringVotingNotAllowed");
      });
    ...
    describe("MEDIUM-02 Fix: Upgrade restrictions after activation", function () {
      it("Should block upgrades after activation (during voting)", async function () {
        ...
        ).to.be.revertedWithCustomError(jurySC, "UpgradesDuringVotingNotAllowed");
      });

      it("Should block upgrades after voting ends but before lock", async function () {
        // Close voting
        await jurySC.connect(owner).closeManually();
      ...
      it("Should allow upgrades before activation", async function () {
        ...
        await juryFresh.waitForDeployment();

        await pobFresh.connect(owner).transferOwnership(await juryFresh.getAddress());

        // Setup but don't activate
        await juryFresh.connect(owner).registerProject(project1.address);
        await juryFresh.connect(owner).setDevRelAccount(devRel.address);
