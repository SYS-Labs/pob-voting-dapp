...
export interface UseIterationMetadataManagerReturn {
  ...
  submitMetadata: (formData: IterationMetadataForm) => Promise<void>;
  clearPending: () => void;
}

/**
 * Hook for managing iteration metadata updates
 * Handles CID loading, metadata submission, and confirmation polling
 *
 * Note: Only the contract owner can update iteration metadata
 */
export function useIterationMetadataManager(
...
): UseIterationMetadataManagerReturn {
  ...
  useEffect(() => {
    if (!currentCID || !chainId || !contractAddress) return;
    ...
    loadMetadata();
  }, [currentCID, chainId, contractAddress]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        console.log('[useIterationMetadataManager] Loaded pending state from localStorage:', pending);
      }
    } catch (error) {
  ...
  }, [pendingTxHash, pendingCID, chainId, contractAddress]);

  // Submit metadata update using role-gated flow
  const submitMetadata = useCallback(async (formData: IterationMetadataForm) => {
    ...
    try {
      ...
      const metadata: IterationMetadata = {
        ...
        chainId,
        votingMode: 0, // Will be overridden by contract state
      };
      ...
      // API will verify the signer is the contract owner before uploading to IPFS
  ...
  }, [signer, iterationNumber, round, chainId, contractAddress, votingActive, pendingConfirmations]);
